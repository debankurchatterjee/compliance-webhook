...
REGISTRY ?= tca-dev-test-docker-local.artifactory.eng.vmware.com
TAG ?= v1.1.4
...
install : generate-tls-certs docker-build
	kubectl create secret tls webhook-server-tls  --cert $(CERT_OUT) --key $(CERT_KEY) -n $(WEBHOOK_SERVER_NAMESPACE)
	sed -i '' 's|tca-dev-test-docker-local.artifactory.eng.vmware.com/webhook-server:latest|$(REGISTRY)/$(APP_NAME):$(TAG)|g' $(WEBHOOK_SERVER_MANIFEST)
